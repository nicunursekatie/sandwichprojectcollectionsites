const HostAvailabilityApp = () => {
  ...
  const getInitialHosts = () => {
    ...
    return [
      ...
      { id: 23, name: 'Allison T.', area: 'Sandy Springs', neighborhood: 'Declaire (just a smidge outside Perimeter)', lat: 33.91549, lng: -84.3968077, phone: '770.355.8876', hours: '9:30 am to 7 pm', openTime: '09:30', closeTime: '19:00', notes: '', available: true },
      ...
      { id: 26, name: 'Stacey & Jack G.', area: 'Virginia Highland', neighborhood: 'Virginia Highland/Morningside/Midtown HS', lat: 33.77723595, lng: -84.362274174978, phone: '404.451.7648', hours: '8am-noon / 5-8pm', openTime: '08:00', closeTime: '12:00', notes: 'Also available 5-8pm', available: true },
...
This is safe because your API key is already restricted to only the Geocoding API.`);
  ...
  const getHostAvailability = (host) => {
    ...
    const now = new Date();
    const currentDay = now.getDay(); // 0 = Sunday, 3 = Wednesday, 4 = Thursday
    const currentTime = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;

    const isWednesday = currentDay === 3;
    const isThursday = currentDay === 4;
    const hasThursdayHours = host.thursdayOpenTime && host.thursdayCloseTime;

    // If it's Thursday and host has Thursday hours
    if (isThursday && hasThursdayHours) {
      if (currentTime < host.thursdayOpenTime) {
        const openDate = new Date();
        const [openHour, openMin] = host.thursdayOpenTime.split(':');
      ...
      } else if (currentTime >= host.thursdayOpenTime && currentTime < host.thursdayCloseTime) {
        const closeDate = new Date();
        const [closeHour, closeMin] = host.thursdayCloseTime.split(':');
    ...
    if (!isWednesday && !(isThursday && hasThursdayHours)) {
    ...
    if (currentTime < host.openTime) {
      ...
      // Closed on Wednesday - check if they have Thursday hours
      if (hasThursdayHours) {
        return {
          status: 'closed-opens-thursday',
          message: `Closed today - opens tomorrow (Thursday) at ${formatTime(host.thursdayOpenTime)}`,
          color: '#FBAD3F'
        };
      } else {
        return {
              ...
              <div className="p-8">
                ...
                <form onSubmit={(e) => {
                  ...
                  const hostData = {
                    ...
                    thursdayOpenTime: formData.get('thursdayOpenTime') || undefined,
                    thursdayCloseTime: formData.get('thursdayCloseTime') || undefined,
