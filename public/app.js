...
const HostAvailabilityApp = () => {
  ...
  const applyCollectionAvailability = helperRefs.applyCollectionAvailability || ((hosts, dateStr) => {
    ...
    return list.map((host) => {
      ...
      if (next?.available && next.alternateFor) {
        ...
        if (primary && isCollecting(primary)) next = { ...next, available: false };
      }
      return next;
    });
  });
  ...
  const allHostsForDisplay = applyCollectionAvailability(rawHosts, activeCollectionWednesdayStr);
  ...
  if (currentPage === 'specialcollections') {
    ...
    return (
      <div className="min-h-screen p-3 sm:p-4 md:p-6 lg:p-8" style={{backgroundColor: '#FFF5F7'}}>
        ...
        {/* Admin Modal for Special Collections Page */}
        {showAdmin && (
            ...
            <div className="modal-content bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto premium-card-header" onClick={e => e.stopPropagation()}>
              <div className="p-4 sm:p-8">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-bold" style={{color: '#236383'}}>🔧 Admin: Manage Special Collections</h2>
                  <button
        ...
        {/* Special Collection Edit Modal for Special Collections Page */}
        {editingSpecialCollection && (
            ...
            <div className="modal-content bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="p-4 sm:p-6">
                  ...
                  <div className="border-t pt-4 mb-4">
                    ...
                    <div className="flex flex-wrap items-end gap-3">
                      <div>
                        ...
                        <input type="time" id="bulkOpenTime2" defaultValue="08:00" className="px-3 py-2 rounded-lg border-2" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium mb-1" style={{color: '#236383'}}>Close Time</label>
                        <input type="time" id="bulkCloseTime2" defaultValue="18:00" className="px-3 py-2 rounded-lg border-2" />
                      </div>
                      <button
                        type="button"
                        onClick={() => {
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto mb-4">
                      {editingSpecialCollection.hosts.map((host) => (
                        <div key={host.id} className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 rounded-lg gap-2" style={{backgroundColor: '#f0f9fa'}}>
                          ...
                          <div className="flex gap-2">
                            <button type="button" onClick={() => setEditingSpecialHost(host)} className="text-sm px-2 py-1 rounded" style={{backgroundColor: '#007E8C', color: 'white'}}>Edit</button>
                            <button type="button" onClick={() => removeSpecialHost(host.id)} className="text-sm px-2 py-1 rounded" style={{backgroundColor: '#A31C41', color: 'white'}}>Remove</button>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="flex flex-col sm:flex-row justify-end gap-3 mt-4 pt-4 border-t">
                  <button type="button" onClick={() => { setEditingSpecialCollection(null); setEditingSpecialHost(null); }} className="px-6 py-2 rounded-lg font-medium" style={{backgroundColor: '#f0f0f0'}}>Cancel</button>
                  <button
                    type="button"
                    onClick={() => {
        ...
        {/* Special Host Edit Modal for Special Collections Page */}
        {editingSpecialHost && (
              ...
              <div className="p-4 sm:p-6">
                ...
                <form onSubmit={(e) => {
                  ...
                  <div className="flex flex-col sm:flex-row justify-end gap-3 pt-3">
                    <button type="button" onClick={() => setEditingSpecialHost(null)} className="px-4 py-2 rounded-lg font-medium" style={{backgroundColor: '#f0f0f0'}}>Cancel</button>
    ...
    <div className="min-h-screen p-3 sm:p-4 md:p-6 lg:p-8" style={{overflowX: 'hidden', width: '100%', maxWidth: '100vw'}}>
      <div className="max-w-5xl mx-auto" style={{width: '100%', maxWidth: '100%'}}>
        ...
        <div className="bg-white rounded-2xl premium-card-header p-4 sm:p-6 mb-4 sm:mb-6">
          ...
          {/* Saved Host Card - Compact but informative */}
          {favoriteHostId && (() => {
            const favoriteHost = allHosts.find(h => h.id === favoriteHostId);
                          ...
                          <div key={host.id} className="flex flex-col gap-3 p-4 rounded-xl hover:bg-gray-50 border border-gray-200">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
                              <div className="flex-1">
                                ...
                                {hasFiniteDistance(host.distance) && (
          ...
          {/* Map View. Kept mounted in list mode so Google Maps is not destroyed. */}
            ...
            <div className="p-4 sm:p-6 border-b" style={{borderColor: 'rgba(71, 179, 203, 0.15)'}}>
              ...
              <div className="relative">
                ...
                {mapTooltip && (
                  <div
                    className="absolute bg-white rounded-xl shadow-2xl p-3 z-10 border-2 overflow-y-auto"
                    style={{
                      top: '10px',
                      left: '50%',
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <div className="relative" data-map-tooltip-menu>
                        ...
                        {mapTooltipMenuOpen && (
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg shadow-xl border-2 z-[100] overflow-y-auto" style={{borderColor: '#007E8C', width: '100%', maxWidth: '100%', maxHeight: '200px'}}>
                            <button
                              type="button"
                              onClick={(e) => {
                                e.preventDefault();
                            ...
                            >
                              <i className="lucide-route w-5 h-5 flex-shrink-0" style={{color: '#007E8C'}}></i>
                              <div className="font-semibold text-sm" style={{color: '#236383'}}>Show Directions In-App</div>
                            </button>
                            <button
                              type="button"
                              onClick={(e) => {
                              ...
                              className="w-full px-4 py-3 hover:bg-gray-50 flex items-center gap-3 transition-colors border-t" style={{borderColor: '#e0e0e0'}}
                            >
                              <i className="lucide-map w-5 h-5 flex-shrink-0" style={{color: '#007E8C'}}></i>
                    ...
                    <div className="p-5 md:p-6">
                      ...
                      <div className="flex items-center gap-2 flex-wrap mb-4">
                        ...
                        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm" style={{backgroundColor: '#47B3CB', color: '#fff'}}>
                          <span style={{opacity: 0.85}}>Area:</span>
                          <span className="font-semibold">{host.area}</span>
                        </span>
                        {host.neighborhood && (
                          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm" style={{backgroundColor: '#47B3CB', color: '#fff'}}>
                            <span style={{opacity: 0.85}}>Neighborhood:</span>
                      ...
                      {userCoords && hasFiniteDistance(host.distance) && (
